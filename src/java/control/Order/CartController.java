...
public class CartController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(orderDao.checkOrder1(acc.getId())){
            ...
            orderDao.addOrder(acc.getId(), acc.getAddress(), acc.getPhone());
            order = orderDao.getOrder(acc.getId());
        }
        ...
        ArrayList<OrderDetail> listHistoryOrder = orderDao.getHistory(acc.getId());
        if(listHistoryOrder.size() == 0)
...
//        p.print(listOrderDetail);
        request.setAttribute("order", listOrderDetail.size());
